...
async def get_all_brands(session: SessionDep, user_id: UUID = None, current_user: UserReturn = Depends(get_current_active_user)) -> List[BrandReturn]:
    if user_id:
...
async def get_brand_by_id(brand_id: UUID, session: SessionDep, current_user: UserReturn = Depends(get_current_active_user)) -> BrandReturn:
...
async def update_brand(brand_id: UUID, brand: BrandUpdate, session: SessionDep, current_user: UserReturn = Depends(get_current_active_user)) -> BrandReturn:
    return await BrandService.update_brand(brand_id, brand, session)


@brand_router.delete("/{brand_id}", response_model=BrandReturn)
async def delete_brand(brand_id: UUID, session: SessionDep, current_user: UserReturn = Depends(get_current_active_user)) -> BrandReturn:
