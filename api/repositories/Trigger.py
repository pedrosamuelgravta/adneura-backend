...
class TriggerRepository:
    ...
    @staticmethod
    async def get_trigger_by_id(trigger_id: UUID, session: SessionDep) -> Trigger | None:
        return session.exec(select(Trigger).where(Trigger.id == trigger_id)).first()

    @staticmethod
    async def get_triggers_by_brand_id(brand_id: UUID, session: SessionDep) -> List[Trigger]:
        return session.exec(select(Trigger).where(Trigger.brand_id == brand_id)).all()

    @staticmethod
    async def get_all_triggers_by_audience(audience_id: UUID, session: SessionDep) -> List[Trigger]:
        return session.exec(select(Trigger).where(Trigger.audience_id == audience_id)).all()
    ...
    async def delete_all_triggers_by_brand_id(
        ...
        existing_triggers = session.exec(
            select(Trigger).where(Trigger.brand_id == brand_id)).all()
